...
public class CommandParameterBinder {
    ...
    private Object bind(CommandExecutionContext context,
        ...
        switch (binding.getRole()) {
            case CONTEXT:
                return context;
            case SENDER:
                return bindSender(context.getSender(), binding.getMetadata());
            case INJECTED:
    ...
    private Object bindSender(CommandSender sender, CommandParameterMetadata parameter) {
        if (!parameter.getValueType().isInstance(sender)) {
    ...
    private Object bindParsed(CommandExecutionContext context,
        ...
        if (resolver == null) {
            throw new IllegalStateException("No command argument resolver found for type " + parameter.getValueType().getName());
        }
